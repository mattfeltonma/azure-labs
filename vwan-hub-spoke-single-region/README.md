...
The lab deploys an Azure Virtual WAN with a single VWAN hub. The VWAN hub is deployed with a VPN Gateway.
...
Routing is configured such that north/south/east/west traffic flows through the VMs in the mediation virtual network. Traffic to and from on-premises flows through the VWAN hub using it as a routing layer.
